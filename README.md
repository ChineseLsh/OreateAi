...
LuckMail provider 支持项目接码和私有邮箱库存两种模式。提供给 OreateAI 的邮箱必须为不带 `+` 别名的独立地址。

默认使用 LuckMail 的 Grok 项目接码库存，明确请求 `ms_imap` 类型的 `outlook.com` 邮箱，并通过订单接口轮询 OreateAI 验证值。可将 `LUCKMAIL_MODE` 设为 `private_inventory`，恢复私有邮箱库存 + Microsoft OAuth/IMAP 模式。
...
| `LUCKMAIL_MODE` | `project_order` | `project_order` 使用项目接码；`private_inventory` 使用私有邮箱库存 |
