...
自有邮箱池支持批量导入 Microsoft OAuth/IMAP 邮箱，格式为 `email----password----client_id----refresh_token`。导入凭据仅保存在本地 `data.db`，列表 API 和页面不会返回密码、client ID 或 refresh token。注册时选择 `self_pool`，系统会原子领取一个可用邮箱；失败后标记为已使用，需在页面明确重新启用。
